package com.matlab.eclipse.mconsole.views;
...
import com.matlab.eclipse.mconsole.internal.ui.preferences.ThemeConstants;
import com.matlab.eclipse.mconsole.internal.ui.preferences.ThemeWrapper;

public class CommandHistoryView extends ViewPart implements
	IPropertyChangeListener {
    private TableViewer viewer;
    public static String VIEW_ID = "com.matlab.eclipse.mconsole.views.CommandHistoryView";
	...
	public void propertyChange(PropertyChangeEvent event) {
		...
		((ITheme) event.getNewValue()).getFontRegistry().addListener(
		    ...
		    ThemeConstants.COMMANDHISTORY_FONT)
		    || event.getProperty().equals(
			    ThemeConstants.COMMANDHISTORY_BACKGROUND_COLOR)
		    || event.getProperty().equals(
			    ThemeConstants.COMMANDHISTORY_FOREGROUND_COLOR)
		    | event.getProperty().equals(
			    ThemeConstants.COMMANDHISTORY_DATE_COLOR)) {
		ThemeWrapper theme = MConsolePlugin.getDefault()
			.getCurrentTheme();
		viewer.getTable().setFont(
			theme.getFont(ThemeConstants.COMMANDHISTORY_FONT));
		viewer.refresh();
	    }
	}
    };
	...
	com.matlab.eclipse.meditor.Activator.getDefault().getPreferenceStore()
	...
	pasteEntryAction = new Action() {
	    ...
	    @Override
	    public void run() {
		Clipboard clipboard = new Clipboard(CommandHistoryView.this
		...
		for (int i = 0; i < transferDatas.length; i++) {
			...
			MConsolePlugin.getDefault().addCommandHistoryEntry(
				new CommandHistoryEntry(plainText));
			update();
			break;
		    }
		}
	...
	cutEntryAction = new Action() {
		...
		try {
			...
			Clipboard clipboard = new Clipboard(
				CommandHistoryView.this.getSite().getShell()
					.getDisplay());
			...
			clipboard.dispose();
			MConsolePlugin.getDefault().deleteCommandHistoryEntry(
				entry);
			update();
		    }
