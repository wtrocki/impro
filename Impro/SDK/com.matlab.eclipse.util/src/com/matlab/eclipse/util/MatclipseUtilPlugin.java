 ...
 *******************************************************************************/
package com.matlab.eclipse.util;
...
public class MatclipseUtilPlugin extends AbstractUIPlugin {
    ...
    // The plug-in ID
    public static final String PLUGIN_ID = "Matclipse.util";
	...
	disp.asyncExec(new Runnable() {
		...
		if (shell != null) {
		    MultiStatus errorStatus = logStatus(message, t,
			    IStatus.ERROR);
		    logError(errorStatus);
		    ErrorDialog.openError(shell, "Matclipse error", message,
			    errorStatus);
		}
	    }
	});
	...
	MultiStatus status = new MultiStatus(getPluginID(), code, message, t);
	String symbolicName = getDefault().getBundle().getSymbolicName();
	status.add(new Status(code, getPluginID(), 0, "Plugin ID: "
		+ symbolicName, null));
	if (t != null) {
	    ...
	    status.add(new Status(code, getPluginID(), 0, stackTraceString
		    .toString(), null));
	}
	return status;
    }
